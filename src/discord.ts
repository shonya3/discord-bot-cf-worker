...
export const validate_interaction_middleware = createMiddleware<{
  ...
  const interaction = await is_valid_discord_interaction_request(
    c.req.raw,
...
export async function is_valid_discord_interaction_request(
  request: Request,
...
export function create_msg_link({
  guild_id,
  channel_id,
  message_id,
}: {
...
export async function fetch_channel_messages({
  channel_id,
  bot_token,
  limit,
}: FetchChannelMessagesArgs): Promise<Array<Message>> {
  try {
